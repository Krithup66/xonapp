...
app.get('/health', (req, res) => {
  res.json({status: 'ok', service: 'trading-journal-service'});
});

// Routes with rate limiting
app.use('/records', readRateLimiter, tradingRoutes);

// Error handler (must be last)
app.use(secureErrorHandler);

app.listen(PORT, () => {
  console.log(`Trading Journal Service running on port ${PORT}`);
});
